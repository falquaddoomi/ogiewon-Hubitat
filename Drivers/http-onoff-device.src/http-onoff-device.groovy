 ...
 *    2023-11-21  falquaddoomi    Copied existing HTTP Momemtary Switch to a new device, HTTP On-Off Device
 * 
 */

metadata {
    definition (name: "HTTP On-Off Device", namespace: "ogiewon", author: "Dan Ogorchock", importUrl: "https://raw.githubusercontent.com/ogiewon/Hubitat/master/Drivers/http-momentary-switch.src/http-momentary-switch.groovy") {
        capability "Switch"
	}
    ...
    preferences {
        ...
        input(name: "devicePath", type: "string", title:"URL Path", description: "Rest of the URL, include forward slash, when triggering 'on' event (or 'off' if off unspecified).", displayDuringSetup: true)
        input(name: "deviceOffPath", type: "string", title:"URL Path (off)", description: "Rest of the URL, include forward slash, when triggering 'off' event (uses 'on' path if unspecified).", displayDuringSetup: true)
        ...
        input(name: "deviceOffMethod", type: "enum", title: "POST, GET, or PUT (off)", options: ["POST","GET","PUT"], defaultValue: "POST", required: true, displayDuringSetup: true)
        ...
        input(name: "deviceOffHeader", type: "string", title:"Device Header (off)", description: "Extra header", displayDuringSetup: true)
        input(name: "deviceBody", type: "string", title:"Body", description: "Body of message", displayDuringSetup: true)
        input(name: "deviceOffBody", type: "string", title:"Body (off)", description: "Body of message (when turning off)", displayDuringSetup: true)
...
def on() {
    runCmd(devicePath, deviceMethod, status="on")
	sendEvent(name: "switch", value: "on", isStateChange: true)
}

def off() {
    runCmd(
        deviceOffPath ? deviceOffPath : devicePath,
        deviceOffMethod ? deviceOffMethod : deviceMethod,
        status="off"
    )
	sendEvent(name: "switch", value: "off", isStateChange: true)
}

def runCmd(String varCommand, String method, String status) {
